...
export const CODE_ONLY_COUNTRY_OPTIONS: readonly CountryOption[] = COUNTRY_CODES.map((code) => ({
  code,
  label: code,
}));

/** Builds browser-localized ISO region labels, falling back to stable alpha-2 codes. */
export function localizeCountryOptions(language: "en" | "ur"): CountryOption[] {
  try {
    ...
    return [...CODE_ONLY_COUNTRY_OPTIONS];
  }
}
